...
for j  in range(1,nt_local):
  ...
  # For enforcing initial condition
  t0 = np.array([tmin])
...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    if (j>1):
      ...
      #f_prev_tt = vmap(jacfwd(jacrev(f_prev,argnums=0),argnums=0))

      f_prev_t = jacrev(f_prev,argnums=0)
      f_prev_tt = jacfwd(f_prev_t,argnums=0)

      ic1 = vmap(g)(t0).reshape(-1,1) - vmap(f_prev)(t0).reshape(-1,1)
      #ic2 = f_t(t0).reshape(-1,1) - f_prev_t(t0).reshape(-1,1)
      #ic3 = f_tt(t0).reshape(-1,1) - f_prev_tt(t0).reshape(-1,1)
      ic2 = vmap(f_t)(t0).reshape(-1,1) - vmap(f_prev_t)(t0).reshape(-1,1)
      ic3 = vmap(f_tt)(t0).reshape(-1,1) -vmap(f_prev_tt)(t0).reshape(-1,1)
    else:
      ic1 = vmap(g)(t0).reshape(-1,1) - 0.0
      # ic2 = f_t(t0).reshape(-1,1) - 1.0
      # ic3 = f_tt(t0).reshape(-1,1) - 1.0
      ic2 = vmap(f_t)(t0).reshape(-1,1) - 1.0
      ic3 = vmap(f_tt)(t0).reshape(-1,1) - 1.0
    ...
    # Return mean loss
    return jnp.mean(lambda_t*loss_eq**2) + 10.0*(jnp.mean(ic1**2) + jnp.mean(ic2**2) + jnp.mean(ic3**2))
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('scspinns_L2_output_jerk_eqn.txt', 'a'))
...
plt.savefig('scspinns_jerk_eqn_3D.pdf', bbox_inches='tight')
...
plt.savefig('scspinn_jerk_eqn_pos.pdf', bbox_inches='tight')

# Save the results into txt file
np.savetxt('scspinns_jerk_eqn.txt', np.c_[t_final,gp_final])
...
plt.savefig('scspinns_jerk_eqn_convergence.pdf', bbox_inches='tight')
#plt.show()
