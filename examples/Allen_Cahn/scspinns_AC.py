...
# reference data
data = io.loadmat('../../data/Allen_Cahn/AC.mat')  
...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    if (j>1):
      ...
      ic1 = vmap(g)(t0[:,0],t0[:,1]).reshape(-1,1) - (t0[:,1]*t0[:,1]*jnp.cos(jnp.pi*t0[:,1])).reshape(-1,1)    
    ...
    # Equation loss
    loss_eq = f_t(tin,xin).reshape(-1,1) - lambda1*f_xx(tin,xin).reshape(-1,1) + lambda2*fval**3 -lambda2*fval 
    # Return mean loss
    return jnp.mean(lambda_t*loss_eq**2) + 100.0*jnp.mean(ic1**2)
...
plt.savefig('scspinns_AC_eqn_timestep.pdf')
#plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('scspinns_L2_output_AC.txt', 'a'))
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('scspinns_AC_contour.pdf', bbox_inches='tight')
...
plt.savefig('scspinns_AC_convergence.pdf', bbox_inches='tight')
#plt.show()
