...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    if (j>1):
      ...
      g = lambda t,x: f(params, t, x)*( 1.0 - AuxN(t,tmin,tmax)) + (x**2*jnp.cos(jnp.pi*x))*AuxN(t,tmin,tmax)
  ...
  if (j>1):
    ...
    gp = lambda t,x: f(params, t, x)*( 1.0 - AuxN(t,tmin,tmax)) + (x**2*jnp.cos(jnp.pi*x))*AuxN(t,tmin,tmax)
...
plt.savefig('hcspinns_AC_eqn_timestep.pdf')
#plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('hcspinns_L2_output_AC.txt', 'a'))
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('hcpinns_AC_contour.pdf', bbox_inches='tight')
...
plt.savefig('hcspinns_AC_convergence.pdf', bbox_inches='tight')
#plt.show()
