...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    if (j>1):
      ...
      ic1 = vmap(g)(t0[:,0],t0[:,1]).reshape(-1,1) - (jnp.cos(jnp.pi*t0[:,1])).reshape(-1,1)    
  ...
  def resnet_update_bfgs(params, inputs):
    ...
    solver = LBFGS(fun=objective, value_and_grad=False, has_aux=False, maxiter=500, tol=1e-6, stepsize=0)
...
plt.savefig('scspinns_KdV_eqn_timestep.pdf')
#plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('scspinns_L2_output_KdV.txt', 'a'))
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('scspinn_KdV_contour.pdf', bbox_inches='tight')
...
plt.savefig('Scspinns_KdV_convergence.pdf', bbox_inches='tight')
#plt.show()
