...
# Not using Fourier encoding for wave equation
input_size = 2

layer_sizes = [input_size, 32, 32, 32, 32,  1]
step_size = 5e-3
train_iters = 20000
# doesnt converge for bfgs, just use adam
train_iters_lbfgs = 1000
#train_iters_lbfgs = 0
step = step_size
...
xb = random.uniform(key, shape=(nxb,), dtype='float64', minval=xmin, maxval=xmax)
...
# Multiplying this with the time derivative 
cvel_inv_sq = (1.0/cvel**2)
...
for j  in range(1,nt_local):
  ...
  t0 = jnp.stack([jnp.ones(nxb)*tmin, xb], 1)  
  ...
  if(j > 1):
    f_prev = lambda t,x: f(params_prev, t, x)*(x-xmin)*(x-xmax) 
...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    g = lambda t,x: f(params, t, x)*(x-xmin)*(x-xmax) 
    f_t = vmap(jacrev(g,argnums=0))

    if (j>1):
        #f_prev_t = lambda x,t : f_t(params_prev, t, x)
        f_prev_t = vmap(jacrev(f_prev,argnums=0))
        #         t0[:,0] is t, t0[:,1] is x
        ic1 = vmap(g)(t0[:,0],t0[:,1]).reshape(-1,1) - vmap(f_prev)(t0[:,0],t0[:,1]).reshape(-1,1)
        ic2 = f_t(t0[:,0],t0[:,1]).reshape(-1,1) - f_prev_t(t0[:,0],t0[:,1]).reshape(-1,1)
        #ic2 = 0.0

    else:
        ic1 = vmap(g)(t0[:,0],t0[:,1]).reshape(-1,1) - jnp.sin(t0[:,1]).reshape(-1,1)
        ic2 = f_t(t0[:,0],t0[:,1]).reshape(-1,1) - cvel*jnp.sin(t0[:,1]).reshape(-1,1)



    # Partial derivatives with respect to x and t
    f_xx = vmap(jacfwd(jacrev(g,argnums=1),argnums=1))
    f_tt = vmap(jacfwd(jacrev(g,argnums=0),argnums=0))
      
    #zeq =  cvel_inv_sq*f_tt(tin,xin).reshape(-1,1) - f_xx(tin,xin).reshape(-1,1)
    zeq =  f_tt(tin,xin).reshape(-1,1) - cvel**2*f_xx(tin,xin).reshape(-1,1)
    return jnp.mean(lambda_t*zeq**2) + jnp.mean(ic1**2 + ic2**2)
  ...
  opt_init, opt_update, get_params = adam(step_size, b1=0.9, b2=0.999, eps=1e-10)
  ...
  gp = lambda t,x: f(params, t, x)*(x-xmin)*(x-xmax) 
  ...
  if(j == 1):
    ...
    u_comp = f_exact(tin,xin,cvel).reshape(-1)
    x_final = xin
    t_final = tin
  else:
    gp_final = np.append(gp_final, (vmap)(gp)(tin, xin).reshape(-1))
    u_comp = np.append(u_comp, f_exact(tin,xin,cvel).reshape(-1))
    #print(np.shape(gp_final), np.shape(tp), np.shape(xp))
    #gp_final = np.append(gp_final, gp(tp, xp))
  ...
  if(j == 1):
    ...
    plt.plot(x1, vmap(gp)(tmaxp[:,0], tmaxp[:,1]), ':',label='PINN, t=0.5$\pi$')
    plt.plot(x1, f_exact(tmin, x1,cvel), '--',label='Exact, t=0')
    plt.plot(x1, f_exact(tmax, x1,cvel), '-.',label='Exact, t=0.5$\pi$')
    plt.legend()
    plt.xlim([xmin, xmax])

  elif(j == (nt_local-1)/2):
    ax2 = plt.subplot(2, 2, 2)
    plt.plot(x1, vmap(gp)(tmaxp[:,0], tmaxp[:,1]), ':',label='PINN, t=$\pi$')
    plt.plot(x1, f_exact(tmax, x1,cvel), '--',label='Exact, t=$\pi$')
    plt.legend()
    plt.xlim([xmin, xmax])

  elif(j == (nt_local-1)*3/4):
    ax3 = plt.subplot(2, 2, 3)
    plt.plot(x1, vmap(gp)(tmaxp[:,0], tmaxp[:,1]), ':',label='PINN, t=1.5$\pi$')
    plt.plot(x1, f_exact(tmax, x1,cvel), '--',label='Exact, t=1.5$\pi$')
    plt.legend()
    plt.xlim([xmin, xmax])
  elif(j == nt_local-1):
    ax3 = plt.subplot(2, 2, 4)
    plt.plot(x1, vmap(gp)(tmaxp[:,0], tmaxp[:,1]), ':',label='PINN, t=2$\pi$')
    plt.plot(x1, f_exact(tmax, x1,cvel), '--',label='Exact, t=2$\pi$')
...
plt.savefig('scspinns_wave_eqn_timestep_nt1_c1.pdf')
plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('scspinns_L2_output_wave.txt', 'a'))
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('scspinns_wave_contour_nt1_c1.pdf', bbox_inches='tight')
...
plt.savefig('scspinns_wave_convergence_nt1_c1.pdf', bbox_inches='tight')
plt.show()

# Save results in csv format
# Stack the arrays horizontally
stacked_results = np.column_stack((t_final, x_final, gp_final, error))
file_path = 'scspinns_sol.csv'
