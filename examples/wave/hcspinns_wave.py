...
#layer_sizes = [input_size, 16, 32, 32, 32, 16, 1]
...
for j  in range(1,nt_local):
  ...
  if(j > 1):
    f_prev = lambda x,t: f(params_prev, t, x)
...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    if (j>1):
      g = lambda t,x: f(params, t, x)*(1.0-AuxN(t,tmin,tmax))*(x-xmin)*(x-xmax) + f_prev(x,t)*AuxN(t,tmin,tmax)*(x-xmin)*(x-xmax)
    else:
      g = lambda t,x: f(params, t, x)*( 1.0 - AuxN(t,tmin,tmax))*(x-xmin)*(x-xmax) + (jnp.sin(x) + t*cvel*jnp.sin(x))*AuxN(t,tmin,tmax)
    ...
    # Equation loss
    #loss_eq =  cvel_inv_sq*f_tt(tin,xin).reshape(-1,1) - f_xx(tin,xin).reshape(-1,1)
    loss_eq =  f_tt(tin,xin).reshape(-1,1) - cvel**2*f_xx(tin,xin).reshape(-1,1)
  ...
  if (j>1):
    gp = lambda t,x: f(params, t, x)*(1.0-AuxN(t,tmin,tmax))*(x-xmin)*(x-xmax) + f_prev(x,t)*AuxN(t,tmin,tmax)*(x-xmin)*(x-xmax)
  else:
    gp = lambda t,x: f(params, t, x)*( 1.0 - AuxN(t,tmin,tmax))*(x-xmin)*(x-xmax) + (jnp.sin(x) + t*cvel*jnp.sin(x))*AuxN(t,tmin,tmax)
...
plt.savefig('hcspinn_wave_eqn_timestep_nt10_c1.pdf')
plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('L2_output_wave.txt', 'a'))
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('hcspinn_wave_contour_nt10_c1.pdf', bbox_inches='tight')
...
plt.savefig('hcspinn_wave_convergence_nt10_c1.pdf', bbox_inches='tight')
plt.show()
