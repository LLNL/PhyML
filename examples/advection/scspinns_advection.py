...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    # Return mean loss
    return jnp.mean(lambda_t*loss_eq**2) + jnp.mean(ic1**2)
...
plt.savefig('advection_eqn_timestep_scspinn.pdf')
plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('L2_output_advection_scspinn.txt', 'a'))
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('advection_contour_scspinn.pdf', bbox_inches='tight')
...
plt.savefig('Advection_convergence_scspinn.pdf', bbox_inches='tight')
plt.show()

# Save results in csv format
# Stack the arrays horizontally
stacked_results = np.column_stack((t_final, x_final, gp_final, error))
file_path = 'sol_scspinn.csv'
