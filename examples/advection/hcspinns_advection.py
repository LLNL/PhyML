...
t_final = np.array([])
x_final = np.array([])
# Set 0alue for continuity requirement (Ncont = M in manuscript)
Ncont=0
if Ncont ==0:
...
# This is the function we seek to minimize
  ...
  def objective(params, inputs):
    ...
    if (j>1):
      ...
      g = lambda t,x: f(params, t, x)*( 1.0 - AuxN(t,tmin,tmax)) + (jnp.sin(x))*AuxN(t,tmin,tmax)

    # Partial derivatives with respect to x and t
    f_t = vmap(jacfwd(g,argnums=0))
    f_x = vmap(jacfwd(g,argnums=1))
  
    loss_eq = nu_inv*f_t(tin,xin).reshape(-1,1) + f_x(tin,xin).reshape(-1,1)  
  ...
  if (j>1):
    ...
    gp = lambda t,x: f(params, t, x)*( 1.0 - AuxN(t,tmin,tmax)) + (jnp.sin(x) )*AuxN(t,tmin,tmax)

  # Defining ntp and nxp for plotting

  # Using less points so that output files are small in size 
  ntp = int((nt-1)/nts/2)+1
  #ntp = 101
  nxp = 101
  ...
  if(j == 1):
    ...
    u_comp = f_exact(tin,xin).reshape(-1)
    x_final = xin
    t_final = tin
  else:
    gp_final = np.append(gp_final, (vmap)(gp)(tin, xin).reshape(-1))
    u_comp = np.append(u_comp, f_exact(tin,xin).reshape(-1))
  ...
  if(j == 1):
    ...
    plt.plot(x1, f_exact(tmin, x1), '--',label='Exact, t=0')
    plt.plot(x1, f_exact(tmax, x1), '-.',label='Exact, t=0.25')
    plt.legend()
    plt.xlim([xmin, xmax])
    gp_plot = vmap(gp)(tmaxp[:,0], tmaxp[:,1])

  elif(j == (nt_local-1)/2):
    
    ax2 = plt.subplot(2, 2, 2)
    plt.plot(x1, vmap(gp)(tmaxp[:,0], tmaxp[:,1]), ':',label='PINN, t=0.5')
    plt.plot(x1, f_exact(tmax, x1), '--',label='Exact, t=0.5')
    plt.plot(x1, vmap(gp)(tminp[:,0], tminp[:,1]), ':',label='PINN, t=0.25')
    plt.plot(x1, gp_plot, '-.',label='PINN, t=0.25')
    #plt.plot(x1, f_exact(tmin, x1), '-',label='Exact, t=0.25')
    plt.legend()
    plt.xlim([xmin, xmax])
    gp_plot = vmap(gp)(tmaxp[:,0], tmaxp[:,1])
    ...
    plt.plot(x1, f_exact(tmax, x1), '--',label='Exact, t=0.75')
    plt.plot(x1, vmap(gp)(tminp[:,0], tminp[:,1]), ':',label='PINN, t=0.5')
    plt.plot(x1,  gp_plot, '-.',label='PINN, t=0.5')
    #plt.plot(x1, f_exact(tmin, x1), '-',label='Exact, t=0.5')
    plt.legend()
    plt.xlim([xmin, xmax])
    gp_plot = vmap(gp)(tmaxp[:,0], tmaxp[:,1])
  elif(j == nt_local-1):
    ...
    plt.plot(x1, f_exact(tmax, x1), '--',label='Exact, t=1.0')
    plt.plot(x1, vmap(gp)(tminp[:,0], tminp[:,1]), ':',label='PINN, t=0.75')
    plt.plot(x1,  gp_plot, '-.',label='PINN, t=0.75')
    #plt.plot(x1, f_exact(tmin, x1), '-',label='Exact, t=0.75')
...
plt.savefig('advection_eqn_timestep.pdf')
plt.show()
...
print('\nlayer_sizes = ', layer_sizes, '\nlearning rate = ', step_size, '\nAdam iters = ', train_iters, 
      ...
      file=open('L2_output_advection.txt', 'a'))
...
#im = plt.tricontourf(tc, xc, fexgrid, cmap='rainbow')
#ax.set_aspect(1.0)
...
#im = plt.tricontourf(tc, xc, fmgrid, cmap='rainbow')
#ax.set_aspect(1.0)
ax.set(xlim=(tmin0, tmax0), ylim=(xmin, xmax))
...
#im = plt.tricontourf(tc, xc, error, 50, cmap='rainbow', shading='gouraud')
...
cbar = plt.colorbar(im, cax=cax)
cbar.ax.tick_params(labelsize=12)
plt.savefig('advection_contour.pdf', bbox_inches='tight')
...
plt.savefig('advection_convergence.pdf', bbox_inches='tight')
plt.show()
